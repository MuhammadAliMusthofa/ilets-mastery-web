...
describe('QuestionTypeFields — Multiple Choice', () => {
  it('menampilkan satu baris per pilihan', () => {
    ...
    expect(screen.getByLabelText('Teks pilihan B')).toBeInTheDocument();
  });
  ...
  it('menambah pilihan baru dengan huruf berikutnya', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /Tambah pilihan/i }));
...
describe('QuestionTypeFields — True/False/Not Given', () => {
  it('menawarkan tepat tiga pilihan jawaban', () => {
    ...
    expect(screen.getByLabelText('Jawaban benar: TRUE')).toBeInTheDocument();
    expect(screen.getByLabelText('Jawaban benar: FALSE')).toBeInTheDocument();
    expect(screen.getByLabelText('Jawaban benar: NOT_GIVEN')).toBeInTheDocument();
  });

  it('tidak menampilkan editor pilihan jawaban', () => {
    render(<QuestionTypeFields value={draftFor('TRUE_FALSE_NOT_GIVEN')} onChange={vi.fn()} />);

    expect(screen.queryByRole('button', { name: /Tambah pilihan/i })).not.toBeInTheDocument();
  });
});
...
describe('QuestionTypeFields — Short Answer', () => {
  it('menambah kotak isian sekaligus slot jawabannya', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /Tambah kotak isian/i }));
  ...
  it('mengurangi kotak isian sekaligus membuang slot jawabannya', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /Kurangi kotak isian/i }));
  ...
  it('menerima beberapa varian jawaban dipisah koma', async () => {
    ...
    await userEvent.type(screen.getByLabelText('Jawaban diterima untuk kotak 1'), 'colour, color');
...
describe('QuestionTypeFields — Map Labeling', () => {
  it('meminta URL gambar denah', async () => {
    ...
    await userEvent.type(
      screen.getByLabelText(/URL gambar denah/i),
